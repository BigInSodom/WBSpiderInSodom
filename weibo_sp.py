...
class Markdown():
    def __init__(self):
        self.md= open(os.path.split(os.path.realpath(__file__))[0] + os.sep+ "md"+ os.sep +time.strftime("%Y-%m-%d", time.localtime())+".md","a",encoding="utf-8")
        print(time.localtime())
    def md_name(self,name):
        self.md.write("<h1>"+name+"</h1></br>\n")

    def md_id(self,id):
        self.md.write("<h5>"+id+"</h5></br>\n")

    def md_space(self):
        self.md.write("\n***\n")

    def md_time(self,time):
        self.md.write(time+"</br>\n")
    def md_text(self,text):
        self.md.write(text+"</br>\n")
    def md_pic(self,pic):
        self.md.write("<img src=\""+pic+"\" height=\"200px\"></img></br>\n")
    def md_vid(self,vid):
        self.md.write("<video src=\""+vid+"\" height=\"200px\" controls=\"controls\"></video></br>\n")
#end
...
class Weibo(object):
    ...
    def get_user_info(self):
        ...
        info = js['data']['cards'][0]['card_group'][1]
        logger.info(js['data']['cards'][0]['card_group'][1]['item_content'])
        user_info = OrderedDict()
        user_info['id'] = self.user_config['user_id']
        user_info['screen_name'] = info.get('item_name', '')
        logger.info(user_info['screen_name'])
    ...
    def print_user_info(self):
        """打印用户信息"""
        #RECLUSE
        self.md.md_name(self.user['screen_name'])
        self.md.md_id(self.user['id'])
        #end
        # logger.info('+' * 100)
        # logger.info(u'用户信息')
    ...
    def print_one_weibo(self, weibo):
        ...
        try:
            ...
            logger.info(u'原始图片url：%s', weibo['pics'])
             #RECLUSE
            #logger.info(u'原始图片url：%s', weibo['pics'])
            logger.info(u'原始图片url')
            logger.info(u'%s', weibo['pics'].split(","))
            #end
            ...
            logger.info(u'url：https://m.weibo.cn/detail/%d', weibo['id'])
             #RECLUSE
            self.md.md_time(weibo['created_at'])
            self.md.md_text(weibo['text'])
            pic_arr=weibo['pics'].split(",")
            for pic in pic_arr:
                self.md.md_pic(pic)
            vid_arr=weibo['video_url'].split(",")
            logger.info(u'VIDEO：%s', vid_arr)
            logger.info(u'VIDEO_U：%s', weibo['video_url'])
            for vid in vid_arr:
                if vid != "":
                    if vid!="video":
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            )[0] + os.sep + 'weibo' + os.sep +self.user['id']+"-"+self.user['screen_name']
    ...
    def get_pages(self):
        ...
        try:
            ...
            if since_date <= today:
                 # Recluse
                # if id cannot be searched ,define page_count as a constant
                page_count=5
...
def get_config():
    ...
    config_path = os.path.split(
        os.path.realpath(__file__))[0] + os.sep + 'config_sp.json'
...
if __name__ == '__main__':
    ...
    #Recluse
    #by=ByPy()
    #by.upload(localpath= os.path.split(
#            os.path.realpath(__file__))[0] +"/md",
#remotepath=u'/markdown',
#ondup=u'overwrite')
#    os.removedirs( os.path.split(
#            os.path.realpath(__file__))[0] +"/md")
